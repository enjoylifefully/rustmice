...
class GameUI:
    ...
    def draw_grid(self):
        for r, row in enumerate(self.grid):
            for c, cell_type in enumerate(row):
                ...
                if cell_type == "cat":
                    if self.current_step_index > 0 and self.current_step_index % 3 == 0:
    ...
    def draw_hud(self):
        ...
        if self.has_solutions:
            ...
            help_render = self.font.render(help_text, True, WHITE)
            self.screen.blit(
                help_render,
                (
                    SCREEN_WIDTH - help_render.get_width() - 20,
                    hud_y_start + 35,
                ),
            )
...
def run_game(grid, solutions):
    game = GameUI(grid, solutions)
    game.run()
